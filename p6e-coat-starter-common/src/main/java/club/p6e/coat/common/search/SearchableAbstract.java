...
public abstract class SearchableAbstract<I extends SearchableAbstract.Option> extends ArrayList<I> implements Serializable {
        ...
        if (options == null) {
            ...
            for (final Option option : options) {
                if (LEFT_TYPE.equalsIgnoreCase(option.getType())) {
                    ...
                    linked.add(tmp);
                    tmp = new ArrayList<>();
                    linked.getLast().add(new Mixin(tmp));
     ...
     * @return 选项参数
     */
    protected boolean validationOptionsToMappings(SearchableAbstract<?> options, List<Mapper> mappers) {
        final List<Mixin> mixins = extractOptions(options);
        for (final Mixin mixin : mixins) {
            ...
            if (list != null && !list.isEmpty()) {
                for (Mixin mixin1 : list) {
                    
                }
                return validationOptionsToMappings((SearchableAbstract<?>) list, mappers);
            }
        }
